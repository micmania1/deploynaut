...
<% if $CreateProjectProgress %>
	<% with $CreateProjectProgress %>
	...
	<% end_with %>
<% else_if $canCreateProjects %>
	<h4>Create an environment</h4>
	<% if $CreateProjectsMessages %>
		<p class="alert alert-danger">$CreateProjectsMessages</p>
	<% end_if %>

	<% if not $CreateProjectsMessages %>
		<% with CreateProjectForm %>
